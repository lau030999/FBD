...
RESULTADOS="/home/laura/Documentos/FBD/tarea3/TYVS/resultados/test_cool_6.txt"
...
do
  ...
  # Iniciar el servicio de PostgreSQL
    ...
    tiempo=$( { time psql -d tareafbd -U laura -h localhost -c "SELECT region, category_id FROM ( SELECT *, ROW_NUMBER() OVER (PARTITION BY region ORDER BY views DESC) AS rn FROM tablaunion) AS ranked WHERE rn = 1;"  ; } 2>&1 )
