...
RESULTADOS="/home/laura/Documentos/FBD/tarea3/TYVS/resultados/test_cool_3.txt"
...
do
  ...
  # Iniciar el servicio de PostgreSQL
    ...
    tiempo=$( { time psql -d tareafbd -U laura -h localhost -c "SELECT year, views, title FROM (SELECT EXTRACT(YEAR FROM publish_time) AS year, views, title, ROW_NUMBER() OVER (PARTITION BY EXTRACT(YEAR FROM publish_time) ORDER BY views DESC) AS row_num FROM tablaunion) AS ranked_videos WHERE row_num = 1;"  ; } 2>&1 )
